...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
	...
	for (int i = 0; i < num_particles; i++) {
		...
		// Accumulate in list of particles
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
	...
	for (int i=0; i < num_particles; i++) {
		...
		for (int j=0; j < observations.size(); j++) {
			// transform vehicle's observation to map coordinates
			...
			// calculate new weight of each particle using multi-variate Gaussian (& associations)
		...
		weights[i] = weight;
		// sum weights for normalising weights
		weights_sum += weight;
